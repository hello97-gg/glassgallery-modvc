...
const ImageGrid: React.FC<ImageGridProps> = ({ images, user, onImageClick, onViewProfile, onLikeToggle, onLongPress }) => {
  if (images.length === 0) {
    return (
  ...
  return (
    <div className="columns-2 sm:columns-2 md:columns-3 lg:columns-4 xl:columns-5 gap-2 md:gap-3">
      {images.map((image) => (
        <ImageCard 
            key={image.id} 
            image={image}
