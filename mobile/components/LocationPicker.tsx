  ...
  useEffect(() => {
    ...
    const listenerPromise = (async () => {
      try {
        ...
        console.warn("CapApp not available in LocationPicker", err);
      }
    })();
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
      <div className="bg-surface border border-border rounded-2xl shadow-2xl w-full max-w-3xl h-[80vh] flex flex-col overflow-hidden relative">
        ...
        <div className="p-4 border-b border-border flex justify-between items-center bg-surface z-10">
            ...
            <button 
              onClick={onClose} 
              className="p-1.5 bg-white/5 hover:bg-white/10 text-white rounded-full transition-all duration-150 active:scale-90 border border-white/5 flex items-center justify-center"
              title="Close Map"
            >
              <svg xmlns="http://www.w3.org/2050/svg" className="h-5 w-5 text-neutral-400 hover:text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
